...
date: '2019-08-23'
slug: visualizing-the-decrease-of-artic-sea-ice-extent
categories: []
tags:
  - Sea ice
  - Arctic
...
# file <- "ftp://sidads.colorado.edu/DATASETS/NOAA/G02135/south/daily/data/S_seaice_extent_daily_v3.0.csv"
...
Thanks to the NSIDC, the data is [available for download](ftp://sidads.colorado.edu/DATASETS/NOAA/G02135/). For this post, I was interested in visualizing these scientific measurements.

## The data

The data consist in four variables:
...
p <- sea_ice_extent %>%
  ...
  geom_pointrange(aes(ymin = mean_extent - sd_extent, ymax = mean_extent + sd_extent), size = 0.25) +
  # geom_line() +
...
The previous graph has shown that Arctic sea ice is undoubtedly decreasing for the past few decades. But at which rate is it decreasing? Is it the same for all the months? The average decreasing rate of sea ice extent can be calculated with the slope of a linear regression between `year` and `sea ice extent`.

```{r, fig.height=8, fig.asp=NA}
p +
  geom_smooth(method = "lm", se = FALSE) +
  labs(subtitle = str_wrap(
    "The vertical bar at each point show the standard deviation around the mean. The blue lines show the linear regressions.", 90
  ))
```

Lets visualize the yearly average sea ice extent decrease for each month.
...
Another way of visualizing the decrease of sea ice extent consists in using a heat map.
